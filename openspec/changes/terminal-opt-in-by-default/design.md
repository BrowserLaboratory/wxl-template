...
五個面板的掛載條件不變,仍由 `v-show` 依當前作用中的分頁決定顯示。未出現在分頁列的面板永遠不會成為作用中分頁,其容器因此不會取得尺寸;凡以容器尺寸為前提才初始化的面板(`WxlshPanel` 的 xterm 與 `CodeEditorPanel` 的編輯器)便不會初始化。這不是所有面板的通性——例如 `RepeatPanel` 在 `onMounted` 讀取 localStorage 的快照,與尺寸無關。
