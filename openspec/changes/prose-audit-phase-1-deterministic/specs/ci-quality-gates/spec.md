...
3. Install the Phase-1 audit dependencies from `scripts/prose-audit/requirements.txt` (which SHALL declare concrete pinned versions of `pyyaml`, `jsonschema`, `textstat`, `tiktoken`, and `py-readability-metrics`).
...
6. Invoke `python scripts/prose-audit/run.py <files> --phase 1 --no-fuzz --fail-on critical --out audit-runs/prose-phase1-ci/ --json-summary audit-runs/prose-phase1-ci/summary.json`. The intersection from step 5 is the positional argument list; if the intersection is empty the wrapper SHALL exit 0 with a skip message and the job SHALL be green.
...
The job SHALL fail if and only if the `run.py` wrapper exits with a non-zero status. The wrapper SHALL exit non-zero if at least one Phase-1 deterministic checker reports a finding whose `severity` is `critical`; findings of severity `high`, `medium`, `low`, or `suggestion` SHALL NOT, by themselves, cause the wrapper or the job to fail at the `--fail-on critical` threshold.

The Phase-1 deterministic checker set SHALL comprise exactly these 14 checkers, vendored into `scripts/prose-audit/checks/`: `mainland_vocab`, `placeholder_grep`, `duplicate_sentences`, `citation_format`, `readability_metrics`, `lazy_writer_check`, `ai_tells`, `burstiness`, `hedge_density`, `imperative_fog`, `lexical_diversity`, `pronoun_consistency`, `discourse_marker_density`, `repetition_fingerprint`. The job SHALL NOT invoke any Phase 2 sub-agent dispatch, Phase 3 fuzz mutator, Phase 4 humane-signal scoring, or Phase 5 consolidated-findings pass — those phases remain the responsibility of the maintainer's release-time manual run governed by the `prose-audit-outward-docs` capability.
...
#### Scenario: Critical Phase-1 finding blocks merge

- **WHEN** the `prose-audit` job runs and at least one Phase-1 checker reports a finding whose `severity` is `critical` on any audited file
- **THEN** `scripts/prose-audit/run.py` SHALL exit with a non-zero status
...
- **AND** the workflow SHALL NOT invoke any of the 14 Phase-1 checker modules for this run
...
#### Scenario: Phase-2/3/4/5 passes are not invoked by the CI job
...
- **THEN** each per-file subdirectory SHALL contain `audit-report.phase1.json` and SHALL NOT contain `agents/_inputs.json`, `mutators/`, `fuzz/`, `humane-signal.json`, or any artifact produced by Phase 2, Phase 3, Phase 4, or Phase 5 of the full `humane-prose-audit` pipeline
- **AND** the workflow SHALL NOT define any `ANTHROPIC_API_KEY` (or equivalent LLM credential) secret reference for the `prose-audit` job, because Phase 1 is purely deterministic and SHALL NOT invoke any LLM
