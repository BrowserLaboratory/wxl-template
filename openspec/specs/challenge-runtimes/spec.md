# challenge-runtimes Specification

## Purpose

Defines the unified contract for the browser-side runtimes that execute challenge backend code — the Python ASGI runtime (Pyodide) and the PHP runtime (php-wasm). It states the shared lifecycle once (initialize exactly once per challenge session, mount the decrypted virtual filesystem before executing app code, implement each runtime as a composable under `.vitepress/theme/composables`) and then captures each runtime's own behavior: Python's HTTP→WSGI/ASGI translation, response normalization, micropip package installation, FastAPI/`BASE_PACKAGES` support, and cookie/header transport; PHP's php-wasm execution with request superglobals (`$_SERVER`/`$_GET`/`$_POST`/`$_COOKIE`/`$GLOBALS['_RAW_INPUT']`). This capability consolidates the former `python-asgi-runtime`, `php-runtime`, and `fastapi-challenge` specs.
...
<!-- @trace
source: consolidate-backend-runtimes
updated: 2026-05-29
code:
