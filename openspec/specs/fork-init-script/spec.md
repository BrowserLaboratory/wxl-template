# fork-init-script Specification
...
#### Scenario: User identity containing "wxl" is protected from corruption

- **WHEN** the CLI runs with `--repo myorg/wxl-ctf --author wxlfan --rebrand acme`
- **THEN** `package.json` `repository.url` SHALL remain `git+https://github.com/myorg/wxl-ctf.git`, `author` SHALL remain `wxlfan`, and the README clone URL SHALL still reference `myorg/wxl-ctf` — no rename SHALL touch the identity; files carrying the fork's own slug MAY appear in the residual inventory (flagged in the warning as intentional identity, never silently subtracted)

#### Scenario: A --repo slug that is a substring of an unrelated token does not hide it

- **WHEN** the CLI runs with `--repo myorg/wxl-ctf --rebrand acme` against a file referencing a distinct `myorg/wxl-ctf-mirror`
- **THEN** that file SHALL still be listed in the residual inventory (the residual check subtracts nothing), and the run SHALL report "rebrand NOT complete"

#### Scenario: The tool-created deploy.yml is inventoried, and the env var stays a valid identifier

- **WHEN** the CLI runs with `--rebrand my-ctf` and copies `deploy.yml.template` (which contains `wxl`) to `.github/workflows/deploy.yml`
- **THEN** the created `.github/workflows/deploy.yml` SHALL appear in the residual inventory (never omitted because it is written after the walk), and the `WXL_VERIFY_RUNTIME` rename SHALL produce the valid identifier `MY_CTF_VERIFY_RUNTIME` (not `MY-CTF_VERIFY_RUNTIME`)
